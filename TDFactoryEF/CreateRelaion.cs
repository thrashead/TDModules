...
namespace TDFactoryEF
{
    public partial class CreateRelaion : Form
    {
        ...
        {
            foreach (Relation item in TDFactoryEF.Relations.Where(a => a.ForeignTable == TableName).ToList())
            {
        ...
        private void btnKaydet_Click(object sender, EventArgs e)
        {
            if (comboBox1.SelectedIndex >= 0 && comboBox2.SelectedIndex >= 0)
            {
                ...
                {
                    if (TDFactoryEF.Relations.Where(a => a.RelationName == "FK_" + textBox3.Text).ToList().Count <= 0)
                    {
                        List<Relation> listRel = TDFactoryEF.Relations.Where(a => a.ForeignTable == foreignColumn.TableName && a.ForeignColumn == foreignColumn.ColumnName && a.PrimaryTable == primaryColumn.TableName && a.PrimaryColumn == primaryColumn.ColumnName).ToList();

                        if (listRel.Count <= 0)
                        {
                            TDFactoryEF.Relations.Add(new Relation() { RelationName = "FK_" + textBox3.Text, ForeignTable = foreignColumn.TableName, ForeignColumn = foreignColumn.ColumnName, PrimaryTable = primaryColumn.TableName, PrimaryColumn = primaryColumn.ColumnName });
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                if (TDFactoryEF.Relations.Where(a => a.RelationName == listBox1.SelectedItem.ToString()).ToList().Count > 0)
                {
                    TDFactoryEF.Relations.Remove(TDFactoryEF.Relations.Where(a => a.RelationName == listBox1.SelectedItem.ToString()).FirstOrDefault());
                }
