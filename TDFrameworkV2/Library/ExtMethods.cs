...
namespace TDFramework
{
    internal static class ExtMethods
    {
        ...
        internal static string ReturnRealColumnName<T>(this object _column)
        {
            ...
            return propInfo.GetDBColumnName().ToString();
        }
        ...
        internal static string ReturnFirstColumn(this List<PropertyInfo> _props)
        {
            ...
            if (_props.Count > 0)
            {
                firstColumn = _props.FirstOrDefault().GetDBColumnName();
            }

            return firstColumn;
        }

        internal static string ReturnQueryStringForInsert(this List<PropertyInfo> _props, string tableName, string _identity, bool _returnID = false)
        {
            ...
            foreach (PropertyInfo item in _props.ReturnValidProperties())
            {
                if (item.PropertyType.InType())
                {
                    ...
                    {
                        querystring += "[" + item.GetDBColumnName() + "],";
                    }
                }
            }
            ...
            querystring = _returnID == true ? querystring + " Select SCOPE_IDENTITY()" : querystring;
        ...
        internal static ResultBox CleanResultBox(this ResultBox _resultBox)
        {
            _resultBox.DataCount = 0;
            _resultBox.Data = null;
            _resultBox.ErrorLayer = null;
            _resultBox.ErrorMessage = null;
            _resultBox.Result = false;

            return new ResultBox() { QueryString = _resultBox.QueryString };
        }
